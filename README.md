...
agent plays several levels back-to-back: on each level it must eliminate every
enemy, pick up the key that is guarded by another enemy, and reach the exit, carrying its HP and freeze
...
| Entity               | Behaviour                                                                                                                                                                                                                                                                                               |
|----------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Player**           | The agent. Moves one orthogonal tile per turn, or shoots a hitscan fireballl in one of four directions, or triggers freeze. Starts each run at `MAX_HP = 100`.                                                                                                                                          |
| **Melee enemy**      | Walks one tile toward the player every turn using a BFS shortest path (routing around other enemies so that they don't get clogged onto one tile); deals `25` damage on contact. Only `max_active` are on the level at once — the rest wait in reserve and spawn in (≥4 tiles away) as active ones die. |
| **Guard**            | Stationary, always placed next to the key it protects. Stepping onto the key while the guard is alive deals `25` damage and does **not** pick up the key — the guard must be shot first.                                                                                                                |
| **Warlock**          | Ranged. Chases the player: goes as close as `WARLOCK_FIREBALL_RANGE = 3` tiles away from the player, then sidesteps onto the player's row/column and launches a fireball. While a fireball is in flight the warlock stays in place - that is the window to go after the warlock and shoot it.           |
| **Warlock Fireball** | Travels straight, **passes through walls**, `25` damage, despawns after 3 tiles or on hit. Its observation channel fades along its direction of travel (bright = where it is now, dim = where it is heading).                                                                                           |
...
| **Key**              | Must be picked up before the exit can be used.                                                                                                                                                                                                                                                          |
...
| **Freeze**           | A consumable power-up: `FREEZE_CHARGES = 2` per run (**not** refilled between levels), each freezes all enemies for `FREEZE_TICKS = 2` turns. Using it while a warlock is still alive provokes the warlock (damages the player, no reward); once warlock is eliminated it is safe and rewarded.         |
...
even cleared level 1 (it emptied the level in under a fifth of episodes and never
reached level 2). Making level 1 easier than a setup that already worked
(melee + guard, **no warlock**) lets the agent get a first meaningful step towards the win;
level 2 reintroduces the warlock, level 3 adds more melee.
...
| 2 | exit | `0.33` while enemies remain, `0.66` when the level is clear but the key is still needed, `1.0` when finishable this step (level clear **and** key in hand) |
...
| 14 | **nearest-enemy distance map** | normalised multi-source BFS distance to the *closest* living enemy; all-`1.0` once the level is clear |
...
Movement into a wall does nothing. A shot is hitscan: it hits the nearest enemy in
that direction within `SHOOT_RANGE`. Freeze with no charges left also does nothing.
...
| Progress toward the nearest enemy | `±0.05` / tile | `REWARD_ENEMY_STEP`, only while the level is not clear; switches off once cleared |
...
| Level fully cleared | `+0.5` (once/level) | `REWARD_ALL_CLEARED` |
...
Speed bonus on a level clear/win: `max(0, (50 − steps_this_level) / 50)`.
...
it into a harder, riskier next level. A small reward for clearing the level makes finishing
dangerous (more chances to die and get a negative reward), so the agent learns to avoid the exit. Making the clear
reward dominate that next-level risk compels the agent to go to the next level.

Why `REWARD_ENEMY_STEP` exists: the only spatial gradient (channels 5 + the goal
reward) points at the key/exit, but the exit is closed until every enemy is dead.
Nothing pulled the agent toward the enemies it had to kill, so hunting the guard
and the warlock was left to blind exploration and rarely happened. This
field encourages the agent to "go and fight"; it is kept below
`REWARD_GOAL_STEP`, is measured over a fixed enemy snapshot per move (so it never
rewards/penalises a kill), and disables itself once the level is clear.

### Termination, truncation and level transitions

- **Win:** clearing the final level (level clear + key + on exit). `game.won = True`.
- **Loss:** HP hits 0 (enemy, fireball or spike), or getting hit while grabbing the key past the guard.
...
seated next to the key, spikes can only be placed on cells walled in on ≤1 side, and
a reachable potion is placed on levels after the first. A level is re-generated until
all of the following hold:

- exit, key and (if present) warlock are reachable from the player;
- the guard is reachable along a single axis **without** stepping on the key (which would trigger it to attack the player);
...
- **`train.py`** — local. Interactively confirms the target directory and step
  ...
  `total_timesteps` value near the top of the file is edited per run (set to small value for a test run - raise it before a real run).
...
- **`diagnose(model_path, n_episodes)`** — a richer headless breakdown of *where* the agent stalls, because win/lose/truncated is too coarse to see progress. It reports:
  ...
  - how often it **emptied the level** and **picked up the key** at least once;
  - of truncated episodes, how many ended standing in a finishable state (level clear + key) — the only true "afraid of the exit" case.
...
- **`main()` / `watch_episodes()`** — records episodes and replays the chosen
  category (won/lost/truncated) in a pygame window with an HP bar, last-move/freeze indicators and play/step controls.
