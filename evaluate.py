...
def watch_episodes(episodes, category, grid_size):
    ...
    class ReplayRenderer(Renderer):
        ...
        def __init__(self, grid_size=10):
            ...
            # Whether the agent has spent its freeze yet in the frame on screen.
            self.freeze_used = False

        def _draw_scene(self, game, extra=None, fireball=None):
            # freeze_available starts True and flips to False the moment the agent
            # activates freeze, so this tracks usage frame by frame.
            self.freeze_used = not game.freeze_available
        ...
        def _draw_button(self):
            ...
            # Freeze indicator on the left: grey "freeze not used" until the agent
            # spends it, then red "FREEZE USED".
            if self.freeze_used:
                freeze_text, freeze_color = "FREEZE USED", (235, 70, 70)
            else:
                freeze_text, freeze_color = "freeze not used", (200, 200, 200)
            freeze = self.font.render(freeze_text, True, freeze_color)
            self.screen.blit(
                freeze,
                (inner.x + 15, inner.y + (inner.height - freeze.get_height()) // 2),
            )

            # Label + skip hint on the right.
            ...
            self.screen.blit(
                text,
                (inner.right - text.get_width() - 15,
                 inner.y + (inner.height - text.get_height()) // 2),
            )
